...
describe('MaxBuy', async () => {
  ...
  it('Doesnt allow Buy more than contributed', async () => {
    ...
    const data = encodeData(sellerContract, 'sell', [eth(51), tokenId, nftContract.address]);
    // buy NFT
    await expect(partyBuy.buy(eth(51), sellerContract.address, data)).to.be.revertedWith("PartyBuy::buy: insuffucient funds to buy token plus fee");
  });

  it('Doesnt allow Buy if cannot pay fee', async () => {
    // encode data to buy NFT
    const data = encodeData(sellerContract, 'sell', [eth(49.5), tokenId, nftContract.address]);
    // buy NFT
    await expect(partyBuy.buy(eth(49.5), sellerContract.address, data)).to.be.revertedWith("PartyBuy::buy: insuffucient funds to buy token plus fee");
  });

  it('Doesnt allow Buy over maxPrice', async () => {
    await contribute(partyBuy, signer, eth(52.5));
    // encode data to buy NFT
    const data = encodeData(sellerContract, 'sell', [eth(101), tokenId, nftContract.address]);
    // buy NFT
    await expect(partyBuy.buy(eth(102.5), sellerContract.address, data)).to.be.revertedWith("PartyBuy::buy: can't spend over max price");
  });

  it('Does allow Buy at exactly maxPrice', async () => {
    // encode data to buy NFT
    const data = encodeData(sellerContract, 'sell', [eth(100), tokenId, nftContract.address]);
    // buy NFT
    await expect(partyBuy.buy(eth(100), sellerContract.address, data)).to.emit(partyBuy, 'Bought');
  });
});
